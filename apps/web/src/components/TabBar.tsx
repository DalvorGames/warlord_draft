...
export function TabBar() {
  ...
  return (
    <nav className="flex shrink-0 border-t border-raised bg-sunk px-2 pt-2.5 pb-[calc(10px+env(safe-area-inset-bottom,16px))]">
      {TABS.map((t) => {
        ...
        return (
          <Link key={t.href} href={t.href} className="flex min-h-11 flex-1 flex-col items-center gap-1.5 py-1.5 no-underline">
            <div className="h-0.5 w-[18px]" style={{ background: on ? "var(--rust)" : "transparent" }} />
