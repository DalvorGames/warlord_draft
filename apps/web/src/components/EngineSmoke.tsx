...
export function EngineSmoke({ seed }: { seed: number }) {
  ...
  useEffect(() => {
    ...
    console.log("[warlord] engine data v%d, %d units", engine.dataVersion, Object.keys(engine.data.units).length);
    console.log("[warlord] smoke battle", { seed, terrain, winner: result.winner, brokeIn: result.brokeInPhase, recap: result.recap });
    setLine(`Engine v${engine.dataVersion} · ${Object.keys(engine.data.units).length} units · smoke battle ${result.winner === "A" ? "won" : "lost"} at ${result.brokeInPhase}`);
  }, [engine, seed]);
