...
### 3.4 Deploy (`Phone-Deploy`) — your general, the pitch, the inspector
Top: **your general as a strip** in his culture's deep/bright — `YOUR GENERAL · INDIA`, name, a one-line read ("Charisma 75 steadies every front. Prefers defensive.") and his four numbers as `COMMAND TACTICS SUPPLY CHARISMA` in a 2×2. Then a hint line ("Tap a token to read it, then tap where it stands. His line stays hidden until battle.") with `SHAPE L · C · R` on the right. The field is a sunk panel with a faint centre circle and a small mono line at the top (`PLAINS · WINGS ×1.10`). Three fronts LEFT · CENTRE · RIGHT (centre 1.5× wider), each showing its cohesion word and number (§4.4) and its tokens. A token is a 40px rounded square with the grade letter (grade fill), the short name under it and a class-tint dot. Tap a token to hold it (rust border); every other front then shows a dashed "Place on the left" overlay button; the bench grows "Bench it". Empty fronts turn `#4A3228` with the cost in rust.

**The panel** between the field and the bench is fixed at 142px and always shows the thing being decided:
- *Nothing held → the doctrine card.* `DOCTRINE · Defensive`, a chip reading `ASHOKA'S WAY · +8% TO ALL` (bone fill) when the plan matches `styleToPlan[general.style]`, else `NOT HIS WAY · NO BONUS` (outlined); six cells `ARROWS CHARGE STEADY CENTRE WINGS COHESION` showing the plan's multipliers as `+15%` (bone) / `−10%` (`#D9724F`) / `—` (faint), straight from `rules.fronts.plans[plan]` (skirmish, contact, steadiness, centre, wing, moraleThreshold); and one plain sentence per doctrine (§4.7). Tapping a doctrine pill changes this card and the cohesion words on the field at once.
- *Token held → the unit inspector.* Name, class badge, culture (in its bright); the six stats with bars in the culture bright, with the keys that count for the unit's natural front lit bone and the rest faint; then a computed sentence (§4.6) and the cohesion word each front would read if the unit stood there.

Below: `THE BENCH · NOT YET ON THE FIELD` with the unplaced tokens; the four doctrine pills (selected = raised fill + rust edge; the general's own doctrine carries a bone dot); the CTA "Give battle" (bone) once all eight are placed, else "N still on the bench" (disabled). Header `?` → Numbers page. Deployment goes to `setDeployment(state, Front[])` in slot order and rides in the run string as `dep=`.

### 3.5 Battle (`Phone-Battle`) — the match report, as a live feed
Header: Back, `4 · THE MATCH REPORT`, Skip. Pinned: both armies' morale as large serif numbers in the two culture brights (his turns rust at ≥ 0.80) around a `ROUT AT 0.80` chip, and the `Key beats / Every beat` toggle. Then the feed. **Beats arrive one at a time and the newest enters at the top**, directly under the pinned header, sliding in from −16px over 320ms; earlier beats push down. The screen never scrolls to follow the action, which is the live-commentary pattern (FotMob, Premier League, DAZN, MLS, F1 all list newest first under a pinned score; chronological top-to-bottom is only used for finished logs you read like a story). The morale numbers in the header tick to the latest beat's values. Auto-play reveals a beat every 1.6s; the bone button reads `Begin`, then `Next beat`, and any tap stops auto-play; `Replay` restarts. When the last beat is in, the button becomes "See where you ranked". Each beat card: gutter label (`SKIRM`, `CONTACT`, `PRESS 1`…), the narration line, and on "Every beat" the three opposed front bars with `+31%` / `HELD` tags, plus both morale values at the right. Key beats (breaks, rout) are lifted: serif text, a tag line (`BOTH HIS WINGS BREAK`), and a tinted card in the winner's colour. Build the beats from `rounds[]` (§8).
...
### 4.7 Doctrine copy (Deploy)
One sentence each, shipped as written; the six cells beside it come from `rules.fronts.plans`:
- Aggressive — "Hit first. The charge lands harder, but your fronts stand less steady and break sooner."
- Defensive — "Hold the line. Fronts stand steadier and take more before they break; your own charge and wings are softer."
- Envelopment — "Win on the wings. Wings press much harder and the charge a little; the centre gives some ground."
- Skirmish — "Shoot first. Arrows count far more and wings a little; the clash and the centre a little less."
The style-match bonus (`rules.styleMatchBonus`, 1.08) is shown as `+8% TO ALL` on the matching doctrine and as a bone dot on its pill.
