...
import type { ContestRecord, RoundRecord } from "./resolveFronts.js";
import type { Front, Side } from "./types.js";
...
export function narrateBeats(r: BattleResult, you: Side = "A"): Beat[] {
  ...
  const rounds: RoundRecord[] = r.rounds.map((rd) =>
    flip ? { ...rd, contests: rd.contests.map(swap), events: rd.events.map(swapEvent), moraleA: rd.moraleB, moraleB: rd.moraleA } : rd,
  );
  ...
  // ----- Skirmish -----
  {
    ...
    beats.push({ kind: "skirmish", round: 0, label: "SKIRMISH", narration: text, short, contests: sk, events: rd.events, moraleYou: rd.moraleA, moraleHim: rd.moraleB, broke: brk, routed: routedIn(rd) });
  }
  ...
  if (ctIdx > 0) {
    ...
    beats.push({ kind: "contact", round: 0, label: "CONTACT", narration: text, short, contests: ct, events: rd.events, moraleYou: rd.moraleA, moraleHim: rd.moraleB, broke: brk, routed: routedIn(rd) });
  }
  ...
  for (const rd of rounds.filter((x) => x.round >= 1)) {
    ...
    beats.push({ kind: "press", round: rd.round, label: `PRESS ${rd.round}`, narration, short, contests: [...rollups, ...press], events: rd.events, moraleYou: rd.moraleA, moraleHim: rd.moraleB, broke: brk, routed: routedIn(rd) });
  }
  ...
  beats.push({ kind: "result", round: 0, label: "RESULT", narration: headline, short, contests: [], events: [], moraleYou: last.moraleA, moraleHim: last.moraleB, broke: { you: [], him: [] }, routed });
